...
INSTANCE_LOCK_FILE = os.path.join(BASE_DIR, "Vidoon2026.lock")
INSTANCE_LOCK = None
...
class AccountAuthDialog(QDialog):
    ...
    def _start_auth_request(self, mode, identifier, password):
        ...
        def worker():
            result = {
                ...
                "msg": "",
                "data": {},
                "title": "登录失败" if mode == "login" else "注册失败",
            }
            try:
                if mode == "login":
                    ...
                    success, msg, register_data = register_account_with_server(identifier, password)
                    if not success:
                        result.update({
                            "success": False,
                            "msg": msg or "注册失败",
    ...
    def _do_register(self):
        ...
        if "@" in identifier:
            ...
            if not re.match(email_pattern, identifier):
                QMessageBox.warning(self, "提示", "请输入正确的邮箱地址")
                return
        else:
            ...
            if not re.match(phone_pattern, identifier):
                ...
                return
        if len(password) < 6:
            QMessageBox.warning(self, "提示", "密码至少需要 6 位")
            return

        self._start_auth_request("register", identifier, password)
...
class VideoDownloader(QMainWindow):
    ...
    def __init__(self):
        ...
        QTimer.singleShot(200, self._initial_check_tools_and_cookie)
    ...
    def _init_paths(self):
        ...
        self.ffplay_path = os.path.join(BASE_DIR, FFPLAY_NAME)
    ...
    def _init_state(self):
        ...
        self.auth_mode = ""
        self.account_info = {}
        self.license_key = load_license_key()
    ...
    def _initial_check_tools_and_cookie(self):
        ...
        if not os.path.exists(self.ffplay_path):
            self.log_handler.log("工具丢失: ffplay.exe 未找到，外部播放备用工具不可用")
    ...
    def _apply_authorization_result(self, auth, log_result=True, show_notice=False):
        ...
        self.auth_mode = auth.get("auth_mode", load_auth_data().get("auth_mode", "license"))
        ...
        if log_result and self.authorized:
            if self.auth_mode == "account":
                ...
                self.log_handler.log(f"状态验证成功（缓存有效至{self.expire_date}）")
        elif log_result:
    ...
    def _load_cached_authorization(self):
        ...
        self.auth_mode = auth_data.get("auth_mode", "license")
        self.account_info = auth_data if self.auth_mode == "account" else {}
        self.authorized = False
        self.expire_date = auth_data.get("expire_date", "")
        self.auth_status = "pending" if (auth_data.get("token") or auth_data.get("license_key")) else "no_local"
        ...
        def worker():
            try:
                ...
                auth = {
                    ...
                    "msg": f"状态验证异常：{exc}",
                    "status": "error",
                    "auth_mode": self.auth_mode or "license",
    ...
    def _update_login_button_text(self):
        if not hasattr(self, "btn_input_license"):
            return
        if self.auth_mode == "account" and (self.authorized or load_auth_data().get("token")):
            self.btn_input_license.setText("退出")
            self.btn_input_license.setToolTip("退出当前账号并刷新状态")
        else:
            self.btn_input_license.setText("账号登录")
            self.btn_input_license.setToolTip("登录账号并同步订阅状态")
    ...
    def _consume_account_download_quota(self, url_count, source_label="下载任务"):
        ...
        self.log_handler.log(f"账号模式下载校验：{source_label}，准备登记 {normalized_count} 个下载任务")

        permission = consume_download_permission(normalized_count)
        ...
        if not self.authorized:
            if self.auth_status == "quota_exceeded":
                remain = self.account_info.get("today_download_remaining", 0)
                QMessageBox.warning(self, "下载额度不足", f"{self.auth_msg}\n今日剩余 {remain} 次")
            else:
    ...
    def _build_ui(self):
        ...
        #
        self.btn_input_license = QPushButton("账号登录")
        self.btn_input_license.setFocusPolicy(Qt.NoFocus)
        self.btn_input_license.setObjectName("topNavButton")
        self.btn_input_license.setFixedHeight(27)
        self.btn_input_license.setMinimumWidth(58)
        self.btn_input_license.clicked.connect(self.prompt_for_license)
        ...
        top_layout.addWidget(self.btn_input_license)
        ...
        self.pages.addWidget(self._page_loading_placeholder("批量提取"))
        ...
        self.nav_items = [
            ...
            {"text": "批量提取", "icon": "主", "tooltip": "批量提取", "page_index": 6},
    ...
    def open_website(self):
        """Internal helper."""
        try:
            webbrowser.open(get_app_value("client.website.home", "https://www.muyanshidai.com"))
            self.log_handler.log("打开官网")
        except Exception as e:
            self.log_handler.log(f"打开官网失败：{e}")
        ...
        for url in event.mimeData().urls():
            ...
            if file_path.endswith('.txt'):
                if self.pages.currentIndex() == 1:
                    self.log_handler.log("视频下载页面不支持导入 TXT 文件，请使用批量提取功能")
            else:
    ...
    def prompt_for_license(self):
        """Internal helper."""
        auth_data = load_auth_data()
        if self.auth_mode == "account" and auth_data.get("token"):
    ...
    def _refresh_texts_after_language(self):
        ...
        try:
            ...
            if hasattr(self, 'btn_input_license'):
                self._update_login_button_text()
            ...
            if page_video_download is not None:
                ...
                page_video_download.input_box.setPlaceholderText("请输入视频链接，每行一个（最多 50 条链接）")
    ...
    def _start_video_download(self, urls, save_path, download_type):
        ...
        if not self.authorized:
            ...
            if reply == QMessageBox.Yes and self.prompt_for_license():
                pass
            else:
                return False
        ...
        if self.auth_mode == "account":
            if not self._consume_account_download_quota(len(urls), "主下载页面批量下载"):
                return False
        ...
        downloader_core = self._create_video_downloader_core(
            update_progress_callback=self._update_total_progress,
        ...
        try:
            with self.queue_lock:
                ...
                if self.total_tasks > 0:
                    average_progress = total_progress / self.total_tasks
                else:
                    average_progress = 0
                
                completed_count = self.completed_tasks + self.failed_tasks
                if self.total_tasks > 0:
                    task_completion_progress = (completed_count / self.total_tasks) * 100
                else:
                    task_completion_progress = 0
                
                final_progress = (task_completion_progress * 0.7) + (average_progress * 0.3)
                final_progress = max(0, min(100, final_progress))
            
            self.signals.progress_signal.emit(final_progress)
        ...
        try:
            with self.queue_lock:
                ...
                if completed_count >= self.total_tasks and queue_empty and no_active_downloads:
                    ...
                    self.signals.progress_signal.emit(100.0)
                    self._reset_current_progress()

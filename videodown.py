...
class VideoDownloadPage(QWidget):
    ...
    def _build_ui(self):
        ...
        # 
        if self.platform == 'YouTube':
            placeholder = "请输入 YouTube 视频链接，每行一个"
        elif self.platform == 'TikTok':
            placeholder = "请输入 TikTok 视频链接，每行一个"
        elif self.platform == 'Instagram':
            placeholder = "请输入 Instagram 视频链接，每行一个"
        else:
    ...
    def update_current_progress(self, progress_info):
        ...
        if progress_type in ('start', 'reset'):
            ...
            success = bool((progress_info.get('data') or {}).get('success'))
            self.current_video_progress = 100.0 if success else 0.0
            self.current_speed = "0 KB/s"
            self.lbl_download_percent.setText(
                f"下载百分比：{self.current_video_progress:.1f}%"
            )
...
class VideoDownloaderCore:
    ...
    def _worker(self):
        ...
        while self.is_downloading:
            ...
            # A new video's visible progress always starts from zero.
            ...
            platform_lock = self.platform_locks.get(identify_platform(url))
            ...
            with self.queue_lock:
                ...
                self.task_status[url] = "completed" if success else "failed"
            ...
            if should_cool_down and self.is_downloading:
                ...
                try:
                    ...
                    if output_files:
                        if len(output_files) > 1:
                            self.log_handler.log(f"[{timestamp}] 本次共生成 {len(output_files)} 个视频文件")
                        for index, output_file in enumerate(output_files, 1):
                            file_size = os.path.getsize(output_file)
                            file_size_mb = file_size / (1024 * 1024)
                            prefix = f"[{index}/{len(output_files)}] " if len(output_files) > 1 else ""
                            self.log_handler.log(
                                f"[{timestamp}] {prefix}文件名: {os.path.basename(output_file)}"
                            )
                            self.log_handler.log(f"[{timestamp}] {prefix}保存路径: {output_file}")
                            self.log_handler.log(f"[{timestamp}] {prefix}文件大小: {file_size_mb:.2f} MB")
                    else:
    ...
    def download_single(
        ...
        try:
            ...
            enable_deno=self.enable_deno,
                ...
                youtube_user_agent=self.config.get("youtube_user_agent", self.config.get("user_agent", "")),
                youtube_use_browser_user_agent=bool(self.config.get("youtube_use_browser_user_agent", True)),
                write_thumbnail=bool(write_thumbnail),
    ...
    def cancel_current_download(self):
        """Stop active subprocesses and prevent queued tasks from starting."""
        self.is_downloading = False

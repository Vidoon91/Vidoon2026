...
class AutoUpdateManager(QObject):
    ...
    def check_for_updates(self, silent=False):
        if self._checking or not self.is_enabled():
            return
        ...
        def worker():
            try:
                ...
                if remote_version and package_url and is_newer_version(remote_version):
                    self.update_available.emit(data, silent)
                elif not silent:
                    self.check_failed.emit("当前已是最新版本。", silent)
            except Exception as exc:
                self.check_failed.emit(str(exc), silent)
            finally:
                self._checking = False
    ...
    def _handle_update_available(self, data, silent):
        ...
        if notes:
            message += f"\n\n更新内容：\n{notes}"
        message += "\n\n软件不会自动更新。请点击“打开浏览器下载”，浏览器会直接下载 GitHub 压缩包，下载完成后手动更新。"

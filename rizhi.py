...
class LogHandler(QObject):
    ...
    def log_download_result(self, platform, download_type, success, strategy, cookie_used, error_source=None, deno_used=False):
        """记录下载结果日志。"""
        mode_text = {"video": "MATERIAL", "audio": "AUDIO"}.get(download_type, "MATERIAL")
        extra = {
            "platform": platform,
        ...
        if success:
            ...
            self.logger.info(message, extra=extra)
            success_text = get_success_message(platform, cookie_used=cookie_used, deno_used=deno_used)
